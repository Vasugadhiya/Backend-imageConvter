...
const convertFile = async (req, res, storagePath) => {
  try {
    ...
    for (const file of req.files) {
      const inputPath = path.join(storagePath, file.filename);
      const outputFilename = `${file.originalname.split(".")[0]}-${Date.now()}.${toFormat}`;
      const outputPath = path.join(storagePath, outputFilename);
      ...
      if (file.mimetype === "application/vnd.openxmlformats-officedocument.wordprocessingml.document") {
        ...
        if (toFormat === "pdf") {
          ...
          switch (metadata.format) {
            ...
            case "dds":
              const ddsImage = await Jimp.read(inputPath);
              const ddsBuffer = await ddsImage.getBufferAsync(Jimp.MIME_PNG);
              const { data, width, height } = await imageJs.Image.load(ddsBuffer);
              const encodedDdsBuffer = imageJs.encode(imageJs.getFormat("DDS"), { width, height, data });
              fs.writeFileSync(outputPath, Buffer.from(encodedDdsBuffer));
              break;
            case "HEIC":
            case "HEIF":
              const heicImageBuffer = await fs.readFile(inputPath);
              const heicImage = await heicConvert({ buffer: heicImageBuffer, format: "HEIF" });
              embeddedImage = await pdfDoc.embedJpg(heicImage);
              break;
            default:
              throw new Error(`Unsupported image format: ${metadata.format}`);
          ...
          const slide = pptx.addSlide();
          slide.addImage({
            data: `data:image/${file.mimetype.split("/")[1]};base64,${imageBuffer.toString("base64")}`,
            x: 0.5,
            y: 0.5,
            w: 8.5,
            h: 6,
          ...
          const image = await Jimp.read(inputPath);
          const buffer = await image.getBufferAsync(Jimp.MIME_PNG);
          const heicImage = await heicConvert({ buffer, format: "HEIC" });
          fs.writeFileSync(outputPath, heicImage);
        } else if (toFormat === "rgb") {
          const image = await Jimp.read(inputPath);
          ...
          await Jimp.read(inputPath).then((image) => {
      ...
      results.push({ originalFilename: file.originalname, outputFilename, downloadLink });
    ...
    return res.status(200).send({ message: "Conversion successful", results });
  } catch (err) {
    console.error("Error during conversion:", err);
    res.status(500).send({ error: "Conversion failed" });
  }
